...
def sort_sum(tuples):  
  ...
  def sumTuble(tuple):
    ...
    for i in tuple:
      temp += i  
    return temp  
  listOfTubles = [i for i in sorted(tuples,key=sumTuble,reverse=True)]
  return listOfTubles
...
def even_tuple(input_tuple):
  # +++your code here+++
  listTuble = sorted(input_tuple)
  listTuble = [i for i in listTuble if (i%2) == 0] 
  myTuple = tuple(listTuble)    
  return myTuple
