...
def extractFemaleNames(filename,letter):
  # +++your code here+++
  f = open(filename,'r')
  #this pattern will output a list of tuples ex: [('1','Jessica'),..]
  female_list = re.findall(r'<tr align=\"right\"><td>(\d+)</td><td>\w+</td><td>(\w+)</td>',f.read())
  count = 0
  #this for loop will convert tuples into strings ex: ('1','Jessica') => 'Jessica 1'
  for i in female_list:
    convertedTuple = "%s %s" %(i[1],i[0])
    female_list[count] = convertedTuple
    count += 1
  #this step will leave the names that starts with the letter given   
  female_list = [j for j in female_list if j[0]==letter]
  female_list.sort() 
  return female_list
...
def extractMaleNames(filename,letter):
  ...
  f = open(filename,'r')
  #this pattern will output a list of tuples ex: [('1','Jessica'),..]
  male_list = re.findall(r'<tr align=\"right\"><td>(\d+)</td><td>(\w+)</td><td>\w+</td>',f.read())
  count = 0
  #this for loop will convert tuples into strings ex: ('1','Jessica') => 'Jessica 1'
  for i in male_list:
    convertedTuple = "%s %s" %(i[1],i[0])
    male_list[count] = convertedTuple
    count += 1
  #this step will leave the names that starts with the letter given   
  male_list = [j for j in male_list if j[0]==letter]
...
def extractDocAndPopularityDate(filename):
  # +++your code here+++
  f = open (filename,'r')
  documentDate = re.search(r'<h1>Popular Names by Birth Year</h1>(\w+\s\d+,\s\d+)</td>',f.read())
  f.seek(0)
  popularityDate = re.search(r'<h3 align=\"center\">(\w+\s\w+\s\d+)</h3>',f.read())
  #when .search is used it returns a object
  #to print or show what's inside we must use .group(no.)
  #the no. inside .group(no.) if not defined, it will show the whole pattern
  #if defined, it will show only the group corrospondimg to that no.
  #since i have only one group in my case and it's the first one, the no. = 1
  Document_Popularity_Date = documentDate.group(1)+ ' and '+ popularityDate.group(1)
  return Document_Popularity_Date
